...
--------------------------------------------------------------------
---- AEROSPACE app -> workspace assignments (from ~/.aerospace.toml)
---- Verify classes on machine: hyprctl clients -j | jq '.[].class'
--------------------------------------------------------------------
hl.window_rule({ match = { class = "^(net\\.imput\\.helium|helium|Helium)$" }, workspace = "name:B" }) -- Arc -> Helium
hl.window_rule({ match = { class = "^([Ss]potify)$" }, workspace = "name:M" })
...
-- macOS 'finder/system preferences float' equivalents already covered by
-- the dolphin + settings float rules above.
