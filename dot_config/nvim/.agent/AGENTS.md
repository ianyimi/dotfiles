# nvim-config — Agent Context

## Agent Directives

These apply to all agents, all platforms, always.

**Chezmoi source — not the live config.** Edits to `lua/`, `after/`, `colors/`,
`init.lua` have ZERO effect until `chezmoi apply`; agents MUST NOT run it — stop,
name the target paths, wait. Profiling before an apply measures the old code.
`.agent/` and all `*.md` are chezmoi-ignored. See `docs/product/dev-processes.md`.
...
- Perf baseline (compare before diagnosing a slowdown): `.agent/docs/perf-baseline.md`
