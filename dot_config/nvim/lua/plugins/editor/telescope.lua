return {
  ...
  opts = function()
    ...
    return {
      ...
      -- Full, untruncated file paths in every LSP picker.
      --
      -- Telescope's entry makers put the path in a FIXED-width column and
      -- truncate it head-first, so same-directory results collapse to an
      -- identical common prefix. (`find_files` is immune: `gen_from_file` has
      -- one unbounded column.) The fix depends on the entry maker:
      --
      -- `gen_from_quickfix` puts the path in a 30-col column and the code text
      -- in `remaining`. Both columns are replaced with unbounded ones, so the
      -- path always renders in full and the code text takes whatever room is
      -- left, clipped by the window edge rather than by a column cap.
      --
      -- `gen_from_lsp_symbols` orders its columns path, symbol, kind -- the path
      -- is never last, so no width setting can make it unbounded. For the
      -- workspace pickers we therefore swap in an entry maker that reorders the
      -- columns to symbol, kind, path, putting the path in `remaining`.
      -- document_symbols is single-buffer, so its path is identical on every
      -- row -- hide it and spend the width on symbol names instead.
      --
      -- `builtin.diagnostics` needs nothing: `gen_from_diagnostics` already
      -- renders the path as its `remaining` column.
      pickers = (function()
        -- A column is truncated only if it carries a `width` key; columns
        -- without one pass through untouched (entry_display.lua:89). Both
        -- families below exploit that to keep the path intact.

        -- Wraps telescope's quickfix entry maker, keeping every entry field it
        -- sets and overriding only `display`, so the path leads at full length
        -- and the code text trails it. Column alignment is deliberately given
        -- up: paths vary in length, so the text starts at a ragged column. Any
        -- fixed path column would reintroduce truncation.
        local function quickfix_path_first(picker_opts)
          local make_entry = require("telescope.make_entry")
          local entry_display = require("telescope.pickers.entry_display")
          local utils = require("telescope.utils")

          local displayer = entry_display.create({
            -- Padded: without a fixed-width column there is no alignment
            -- whitespace to separate the path from the text.
            separator = " ▏ ",
          ...
          local function display(entry)
            local location =
              string.format("%s:%d:%d", utils.transform_path(picker_opts, entry.filename), entry.lnum, entry.col)
            -- Strip indentation and telescope's `… | ` prefix; leading
            -- whitespace would waste the room the text has left.
            local text = (entry.text or ""):gsub(".* | ", ""):gsub("^%s*(.-)%s*$", "%1")
            return displayer({ location, text })
          end

          local base = make_entry.gen_from_quickfix(picker_opts)
        ...
        local locations = { entry_maker = quickfix_path_first({}) }

        -- Mirrors telescope's own `lsp_type_highlight` map, which is file-local
        -- to make_entry.lua and so cannot be reused directly.
        ...
        -- Wraps telescope's symbol entry maker, keeping every entry field it
        -- sets (filename/lnum/col drive the previewer and jump) and overriding
        -- only `display`. All bounded columns use absolute widths, so the
        -- displayer holds no window-dependent state and one shared instance is
        -- safe across pickers and window sizes.
        local function symbols_path_last(picker_opts)
          local make_entry = require("telescope.make_entry")
          local entry_display = require("telescope.pickers.entry_display")
          local utils = require("telescope.utils")
          ...
          local displayer = entry_display.create({
            separator = " ",
            items = { { width = 30 }, { width = 10 }, { remaining = true } },
          })

          local function display(entry)
            local kind = entry.symbol_type or "unknown"
            return displayer({
              entry.symbol_name or "",
              { kind:lower(), kind_highlight[kind] },
              utils.transform_path(picker_opts, entry.filename),
            })
          end

          local base = make_entry.gen_from_lsp_symbols(picker_opts)
          return function(item)
            local entry = base(item)
            if entry then
              entry.display = display
        ...
        local workspace_symbols = { entry_maker = symbols_path_last({}) }

        return {
          -- gen_from_quickfix family
          lsp_references = locations,
          lsp_definitions = locations,
          lsp_type_definitions = locations,
          lsp_implementations = locations,
          lsp_incoming_calls = locations,
          lsp_outgoing_calls = locations,
          -- gen_from_lsp_symbols family
