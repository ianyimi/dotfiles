---
description: Review a file, function, or set of changes against established codebase conventions. Reports issues with file path and line reference — does not fix, just flags.
---

# Review — Code Review Against Codebase Conventions

Review a file, function, or set of changes against the established patterns and conventions in this codebase. Produces a structured list of issues with specific locations and what to fix. If no target was specified, ask for one and then proceed.
...
- `/review the createUser() function`
...
Read the target, then check it against each category below. Report issues with file path and line reference. **Do not fix anything** — your job is to surface problems the developer should address.
...
- Properties that have defaults are required in the resolved type. Properties that are meaningless when absent remain optional in both.
...
**[JSDoc] types.ts:45** — `AdminConfig` is missing `@see AdminConfigInput`. Add it.
...
- **Don't fix, just flag.** The developer makes all code changes.
