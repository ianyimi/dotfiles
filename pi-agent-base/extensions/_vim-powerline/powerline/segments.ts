...
/**
 * Vim mode segment with pi-vim colors
 */
const vimModeSegment: StatusLineSegment = {
  render(ctx: SegmentContext): RenderedSegment {
    const mode = ctx.vimMode;
    ...
    // Apply pi-vim colors: borderMuted for insert, borderAccent for normal
    const colorKey = mode === "insert" ? "borderMuted" : "borderAccent";
    const colored = ctx.theme.fg(colorKey, `\x1b[7m ${label} \x1b[27m`);
