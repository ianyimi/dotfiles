...
description: Inject relevant standards into the current context — auto-suggests based on what you're working on, or accepts explicit paths. Adapts output format for conversation, skill-building, or planning.
---
...
2. **Creating a Skill** — Building a `.pi/skills/` file
3. **Shaping/Planning** — In plan mode, building a spec

**Detection logic:**

- If currently in plan mode OR conversation clearly mentions "spec", "plan" → **Shaping/Planning**
- If conversation clearly mentions creating a skill or building a reusable procedure → **Creating a Skill**
- Otherwise → **Ask to confirm** (do not assume)

**If neither is clearly detected**, use AskUserQuestion:
...
Same as Creating a Skill — offer References vs Copy content choice, then format accordingly.

---

### Step 6: Surface Related Skills (Conversation scenario only)

When in conversation scenario, check if `.pi/skills/` or `.claude/skills/` contains related skills:
