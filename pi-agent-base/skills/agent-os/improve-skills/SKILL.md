---
name: improve-skills
description: Analyze your workflow and codebase for repetitive patterns, then generate or refine Pi skills to automate them. Bridges standards discovery with skill creation.
---

# Improve Skills

Analyze your workflow and enhance your skills based on patterns in your codebase and standards.
...
Check for existing skills in `.pi/skills/` and analyze the codebase for opportunities.

Use AskUserQuestion to understand the user's needs:

```
Let's identify opportunities to improve your Pi agent skills.
...
Generate the skill in `.pi/skills/` (or project-local `.pi/skills/` if project-specific):
...
```
Here's the skill draft for `.pi/skills/[skill-name]/SKILL.md`:
...
```
✓ Skill created: .pi/skills/[skill-name]/SKILL.md

To test it, try:
- Ask the agent to [describe a task that would trigger this skill]
...
- "Add component with tests" — Clear scope
