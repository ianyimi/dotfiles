...
**When this skill is invoked, begin executing immediately. Do not ask the user whether to run it — invoking the skill IS the instruction to run it. If no diagram subject was provided, ask for one and then proceed.**
...
After the render-view-fix loop reaches a passable state, you MUST ask the user before stopping:

> "Is this diagram done for now, or do you want to keep refining it?"

**Do not skip this prompt.** It is the trigger for the self-improvement protocol below.

If the user says done (or any equivalent — "looks good", "that's enough", "ship it", "save it"):
1. Run the self-improvement protocol (see below)
2. Then confirm to the user: "Saved to references. The skill will remember this for future diagrams."

If the user wants to keep refining: continue the render-view-fix loop, then ask again at the next natural stopping point.

---

## Self-Improvement Protocol

This protocol runs **after every completed diagram** (when the user says "done for now"). Its purpose is to make the skill progressively better at producing what this user actually wants, with less back-and-forth over time.

### Step 1: Save the PNG as an example

Copy the final rendered PNG to `references/examples/`. Name it descriptively:
```
references/examples/YYYY-MM-DD-<diagram-topic>.png
```

Example: `references/examples/2026-03-28-vexcms-architecture.png`

### Step 2: Update examples/index.md

Maintain a running index at `references/examples/index.md`. Each entry records:
- Filename
- What type of diagram it is (architecture, data flow, process, etc.)
- Complexity score (1-5, where 5 = most complex/comprehensive)
- One-line note on what made it successful

Format:
```markdown
| File | Type | Complexity | Notes |
|------|------|------------|-------|
| 2026-03-28-vexcms-architecture.png | architecture | 5 | Multi-package monorepo, layered top-down, color-coded by package boundary |
```

### Step 3: Prune stale examples

The examples library should stay lean and high-quality. After adding a new example:
- If there are more than 12 examples, remove the lowest-complexity entry (score 1-2) that is **not** representative of the user's typical requests
- Never remove an example that demonstrates a technique not covered by any other example
- When removing, delete both the PNG and its row from index.md

The goal is a curated set of the most complex, most representative diagrams — not an archive.

### Step 4: Extract and save new learnings

After each diagram session, reflect on what you learned about this user's preferences. Open `references/user-preferences.md` and add any new insights (or update existing ones) in the relevant section. Only add what is **new or surprising** — don't repeat what's already there.

Questions to consider:
- Did the user correct your layout, color, or structure choices?
- Did they ask for more or less detail than you gave?
- Was there a pattern or technique they responded well to?
- What domain are they typically working in?

### Step 5: Read user-preferences.md at the start of every diagram

**At the start of every new diagram session, read `references/user-preferences.md` before doing anything else.** This is how you carry learned preferences forward without the user having to repeat themselves.

---

## User Preferences Reference

Read `references/user-preferences.md` **before starting any diagram**. This file is maintained by the self-improvement protocol and contains learned preferences specific to this user.

If the file doesn't exist yet, create it with the template from the Self-Improvement Protocol.
