...
4. **Re-sync generated artifacts.** After editing skill/reference/standards files, run
   `harness index rebuild` and `harness sync`, then `harness doctor` to confirm health.

5. **Report.** Per file: adopted-as-is / merged (what was taken vs kept) / skipped (why).
...
- Same propose-then-merge model is intended for future file-writing ops (e.g. a `harness fetch`
  mode for `sync` bridges): read the plan, merge by hand, keep project config sovereign.
