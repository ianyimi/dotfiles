...
# -----------------------------------------------------------------------------
# Library Standards Functions (for z3 and similar profiles)
# -----------------------------------------------------------------------------
...
detect_framework() {
    ...
    # Check for TanStack Start/Router
    if grep -q '"@tanstack/start"' "$package_json" 2>/dev/null || \
       grep -q '"@tanstack/react-router"' "$package_json" 2>/dev/null; then
        echo "tanstack"
        return
    fi
...
get_package_version() {
    ...
    local version=$(grep -o "\"$package_name\"[[:space:]]*:[[:space:]]*\"[^\"]*\"" "$package_json" 2>/dev/null | \
                   sed 's/.*"[^"]*"[[:space:]]*:[[:space:]]*"\([^"]*\)"/\1/' | \
                   sed 's/[\^~]//')
    echo "$version"
}
...
# Get YAML array values (for library lists in version.yml)
get_yaml_array() {
    local file=$1
    local key=$2
