...
preserve_product_folder() {
    # Preserve the product/ folder if it exists
    local product_dir="$PROJECT_DIR/agent-os/product"
    local temp_product_dir="/tmp/agent-os-product-backup-$$"

    if [[ -d "$product_dir" ]]; then
        print_verbose "Preserving product/ folder"
        cp -r "$product_dir" "$temp_product_dir"
        PRODUCT_BACKUP="$temp_product_dir"
    fi
}

restore_product_folder() {
    if [[ -n "$PRODUCT_BACKUP" ]] && [[ -d "$PRODUCT_BACKUP" ]]; then
        print_verbose "Restoring product/ folder"
        cp -r "$PRODUCT_BACKUP" "$PROJECT_DIR/agent-os/product"
...
main() {
    ...
    # Preserve product folder before reinstall
    preserve_product_folder
    ...
    # Restore product folder after reinstall
    restore_product_folder
