...
1. **Find the sessions.** Search `.agent/docs/session-log/**/*.log.md` entry titles
   ...
   `.commit*.md` / `.handoff.md` files — a day with several commits numbers them
   `.commit.md`, `.commit.2.md`, `.commit.3.md`, and the ledger's `## HH:MM — <title>`
   section says which one belongs to this name. Nothing found → say so, list the nearest
   titles, stop.
