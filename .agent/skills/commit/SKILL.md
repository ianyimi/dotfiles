...
**This repo carries multiple internal harness installs — sweep all of them.** Beyond root
`.agent/`, dotfiles-managed configs run their own nested install (e.g.
`dot_config/nvim/.agent/`, and others as they're added) with an independent session log,
spec dir, and commit ledger under that config's own `.agent/docs/`. Concurrent omp sessions
across these installs — one per config being worked, sometimes several open at once — is
the normal way this repo is used, not an edge case. A commit run MUST check `git status`
for dirty paths under every nested `.agent/` (not just root), run this skill's log/name
steps against each install's OWN session log, and fold every install's prepared work into
the developer's requested commit shape (their instruction on this run — one commit,
several — always wins over the default one-cluster-per-concern clustering in step 2).
`harness/src/templates/` is upstream, not an install with its own sessions — see
`AGENTS.md` "Repo layout" — its changes get attributed to whichever session ported them.
