...
6. **Commit — by mode.**
   - `message-only`: the message lives in the `.commit.md` file — done. Do NOT run
     `git commit`.
...
7. **Summary.** FIRST line, message-only mode: the commit message file path —
   `.agent/docs/session-log/YYYY/MM/YYYY-MM-DD.commit.md` — so the developer returning to the
   session sees immediately where to copy from; then the full message itself. Agent-commits:
   the new SHA. Then "Updated: [files]. Needs your attention: [list]."
